...
const MeshGradient = () => {
    ...
    useEffect(() => {
        ...
        const animate = () => {
            ...
            orbs.forEach((orb) => {
                ...
                const gradient = ctx.createRadialGradient(orb.x * canvas.width, orb.y * canvas.height, 0, orb.x * canvas.width, orb.y * canvas.height, orb.radius * Math.max(canvas.width, canvas.height));
    ...
    return _jsx("canvas", { ref: canvasRef, className: "mesh-gradient" });
};
